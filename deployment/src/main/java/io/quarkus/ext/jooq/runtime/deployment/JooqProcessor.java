...
import io.quarkus.ext.jooq.runtime.*;
...
class JooqProcessor {
    ...
    private void createDslContextProducerBean(BuildProducer<GeneratedBeanBuildItem> generatedBean,
        ...
        ClassCreator classCreator = ClassCreator.builder().classOutput(classOutput).className(dslContextProducerClassName).superClass(AbstractDslContextProducer.class).build();
        ...
        if (isPresentDialect(defaultConfig)) {
            ...
            if (defaultConfig.datasource.isPresent() && !DataSourceUtil.DEFAULT_DATASOURCE_NAME.equals(defaultConfig.datasource.get())) {
                log.warn("Skip default dataSource name: " + defaultConfig.datasource.get());
            }
            String dsVarName = "defaultDataSource";

            FieldCreator defaultDataSourceCreator = classCreator.getFieldCreator(dsVarName, AgroalDataSource.class).setModifiers(Opcodes.ACC_MODULE);
            ...
            MethodCreator defaultDslContextMethodCreator = classCreator.getMethodCreator("createDefaultDslContext", DSLContext.class);
            ...
            ResultHandle dataSourceRH = defaultDslContextMethodCreator.readInstanceField(FieldDescriptor.of(classCreator.getClassName(), dsVarName, AgroalDataSource.class.getName()), defaultDslContextMethodCreator.getThis());
            ...
            if (defaultConfig.configurationInject.isPresent()) {
                ...
                FieldCreator configurationCreator = classCreator.getFieldCreator(injectVarName, JooqCustomContext.class).setModifiers(Opcodes.ACC_MODULE);

                configurationCreator.addAnnotation(Inject.class);
                configurationCreator.addAnnotation(AnnotationInstance.create(DotNames.NAMED, null, new AnnotationValue[]{AnnotationValue.createStringValue("value", configurationInjectName)}));

                ResultHandle configurationRH = defaultDslContextMethodCreator.readInstanceField(FieldDescriptor.of(classCreator.getClassName(), injectVarName, JooqCustomContext.class.getName()), defaultDslContextMethodCreator.getThis());
                ...
                defaultConfig.configuration.ifPresent(s -> unremovableBeans.produce(new UnremovableBeanBuildItem(new BeanClassNameExclusion(s))));

                defaultDslContextMethodCreator.returnValue(defaultDslContextMethodCreator.invokeVirtualMethod(
                        MethodDescriptor.ofMethod(AbstractDslContextProducer.class, "createDslContext", DSLContext.class, String.class, AgroalDataSource.class, String.class),
                        defaultDslContextMethodCreator.getThis(), dialectRH, dataSourceRH, configurationRH));
            }
        }
        ...
        for (Entry<String, JooqItemConfig> configEntry : jooqConfig.namedConfig.entrySet()) {
            ...
            FieldCreator dataSourceCreator = classCreator.getFieldCreator(dsVarName, AgroalDataSource.class).setModifiers(Opcodes.ACC_MODULE);
            dataSourceCreator.addAnnotation(Inject.class);
            dataSourceCreator.addAnnotation(AnnotationInstance.create(DotNames.NAMED, null, new AnnotationValue[]{AnnotationValue.createStringValue("value", dataSourceName)}));

            MethodCreator namedDslContextMethodCreator = classCreator.getMethodCreator("createNamedDslContext_" + suffix, DSLContext.class.getName());
            ...
            namedDslContextMethodCreator.addAnnotation(AnnotationInstance.create(DotNames.NAMED, null, new AnnotationValue[]{AnnotationValue.createStringValue("value", named)}));
            namedDslContextMethodCreator.addAnnotation(AnnotationInstance.create(DSL_CONTEXT_QUALIFIER, null, new AnnotationValue[]{AnnotationValue.createStringValue("value", named)}));

            ResultHandle dialectRH = namedDslContextMethodCreator.load(namedConfig.dialect);

            ResultHandle dataSourceRH = namedDslContextMethodCreator.readInstanceField(FieldDescriptor.of(classCreator.getClassName(), dsVarName, AgroalDataSource.class.getName()), namedDslContextMethodCreator.getThis());
            ...
            if (namedConfig.configurationInject.isPresent()) {
                ...
                namedConfig.configuration.ifPresent(s -> unremovableBeans.produce(new UnremovableBeanBuildItem(new BeanClassNameExclusion(s))));
